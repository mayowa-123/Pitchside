...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (endpoint === 'h2h') {
      ...
      // GET /matches/{matchId} — Highlightly bundles venue, referee,
      // weather forecast, events (= Timeline), and matchStatistics
      // (= Stats) all into this single response. One call covers three
      // tabs on the client instead of three separate requests.
      if (!matchId) {
        res.status(400).json({ error: 'matchId is required for match' });
        return;
      }
      url = `${BASE_URL}/matches/${encodeURIComponent(matchId)}`;
    } else if (endpoint === 'lineups') {
      // GET /lineups/{matchId} — Highlightly's own docs note lineups
      // aren't confirmed until ~30 min before kickoff (or up to 15 min
      // after), so an empty response here can just mean "too early," not
      // a bug.
      if (!matchId) {
        res.status(400).json({ error: 'matchId is required for lineups' });
        return;
      }
      url = `${BASE_URL}/lineups/${encodeURIComponent(matchId)}`;
    } else {
